...
- This Nautobot App is compatible with Nautobot {{ cookiecutter.min_nautobot_version }} and higher.
...
```shell
pip install {{ cookiecutter.nautobot_app_slug }}
```

To ensure {{ cookiecutter.verbose_name }} is automatically re-installed during future upgrades, create a file named `local_requirements.txt` (if not already existing) in the Nautobot root directory (alongside `requirements.txt`) and list the `{{ cookiecutter.nautobot_app_slug }}` package:

```shell
echo {{ cookiecutter.nautobot_app_slug }} >> local_requirements.txt
```
...
- Append `"{{ cookiecutter.nautobot_app_name }}"` to the `NAUTOBOT_APPS` list.
- Append the `"{{ cookiecutter.nautobot_app_name }}"` dictionary to the `NAUTOBOT_APPS_CONFIG` dictionary and override any defaults.
