...
- `project_name`: the default docker compose project name (default: `{{ cookiecutter.nautobot_app_name }}`)
...
Using **Invoke** these configuration options can be overridden using [several methods](https://docs.pyinvoke.org/en/stable/concepts/configuration.html). Perhaps the simplest is setting an environment variable `INVOKE_{{ cookiecutter.nautobot_app_name.upper() }}_VARIABLE_NAME` where `VARIABLE_NAME` is the variable you are trying to override. The only exception is `compose_files`, because it is a list it must be overridden in a YAML file. There is an example `invoke.yml` (`invoke.example.yml`) in this directory which can be used as a starting point.
...
#14 naming to docker.io/{{ cookiecutter.nautobot_app_slug }}/nautobot:{{ cookiecutter.nautobot_version }}-py3.7 done
```
...
Creating network "{{ cookiecutter.nautobot_app_name }}_default" with the default driver
Creating volume "{{ cookiecutter.nautobot_app_name }}_postgres_data" with default driver
Creating {{ cookiecutter.nautobot_app_name }}_redis_1 ...
Creating {{ cookiecutter.nautobot_app_name }}_docs_1  ...
Creating {{ cookiecutter.nautobot_app_name }}_postgres_1 ...
Creating {{ cookiecutter.nautobot_app_name }}_postgres_1 ... done
Creating {{ cookiecutter.nautobot_app_name }}_redis_1    ... done
Creating {{ cookiecutter.nautobot_app_name }}_nautobot_1 ...
Creating {{ cookiecutter.nautobot_app_name }}_docs_1     ... done
Creating {{ cookiecutter.nautobot_app_name }}_nautobot_1 ... done
Creating {{ cookiecutter.nautobot_app_name }}_worker_1   ...
Creating {{ cookiecutter.nautobot_app_name }}_worker_1   ... done
...
ee90fbfabd77   {{ cookiecutter.nautobot_app_slug }}/nautobot:{{ cookiecutter.nautobot_version }}-py3.7   "nautobot-server rqw…"   16 seconds ago   Up 13 seconds                                               {{ cookiecutter.nautobot_app_name }}_worker_1
b8adb781d013   {{ cookiecutter.nautobot_app_slug }}/nautobot:{{ cookiecutter.nautobot_version }}-py3.7   "/docker-entrypoint.…"   20 seconds ago   Up 15 seconds   0.0.0.0:8080->8080/tcp, :::8080->8080/tcp   {{ cookiecutter.nautobot_app_name }}_nautobot_1
d64ebd60675d   {{ cookiecutter.nautobot_app_slug }}/nautobot:{{ cookiecutter.nautobot_version }}-py3.7   "mkdocs serve -v -a …"   25 seconds ago   Up 18 seconds   0.0.0.0:8001->8080/tcp, :::8001->8080/tcp   {{ cookiecutter.nautobot_app_name }}_docs_1
e72d63129b36   postgres:13-alpine               "docker-entrypoint.s…"   25 seconds ago   Up 19 seconds   0.0.0.0:5432->5432/tcp, :::5432->5432/tcp   {{ cookiecutter.nautobot_app_name }}_postgres_1
96c6ff66997c   redis:6-alpine                   "docker-entrypoint.s…"   25 seconds ago   Up 21 seconds   0.0.0.0:6379->6379/tcp, :::6379->6379/tcp   {{ cookiecutter.nautobot_app_name }}_redis_1
```
...
Stopping {{ cookiecutter.nautobot_app_name }}_worker_1   ...
Stopping {{ cookiecutter.nautobot_app_name }}_nautobot_1 ...
Stopping {{ cookiecutter.nautobot_app_name }}_docs_1     ...
Stopping {{ cookiecutter.nautobot_app_name }}_redis_1    ...
Stopping {{ cookiecutter.nautobot_app_name }}_postgres_1 ...
Stopping {{ cookiecutter.nautobot_app_name }}_worker_1   ... done
Stopping {{ cookiecutter.nautobot_app_name }}_nautobot_1 ... done
Stopping {{ cookiecutter.nautobot_app_name }}_postgres_1 ... done
Stopping {{ cookiecutter.nautobot_app_name }}_redis_1    ... done
Stopping {{ cookiecutter.nautobot_app_name }}_docs_1     ... done
Removing {{ cookiecutter.nautobot_app_name }}_worker_1   ...
Removing {{ cookiecutter.nautobot_app_name }}_nautobot_1 ...
Removing {{ cookiecutter.nautobot_app_name }}_docs_1     ...
Removing {{ cookiecutter.nautobot_app_name }}_redis_1    ...
Removing {{ cookiecutter.nautobot_app_name }}_postgres_1 ...
Removing {{ cookiecutter.nautobot_app_name }}_postgres_1 ... done
Removing {{ cookiecutter.nautobot_app_name }}_docs_1     ... done
Removing {{ cookiecutter.nautobot_app_name }}_worker_1   ... done
Removing {{ cookiecutter.nautobot_app_name }}_redis_1    ... done
Removing {{ cookiecutter.nautobot_app_name }}_nautobot_1 ... done
Removing network {{ cookiecutter.nautobot_app_name }}_default
```
...
So for example, our plugin is named `{{ cookiecutter.nautobot_app_slug }}`, the command would most likely be `docker logs {{ cookiecutter.nautobot_app_name }}_nautobot_1 -f`. You can find the name of all running containers via `docker ps`.
...
```python
namespace = Collection("{{ cookiecutter.nautobot_app_name }}")
namespace.configure(
    {
        "{{ cookiecutter.nautobot_app_name }}": {
            ...
...
Or set the `INVOKE_{{ cookiecutter.nautobot_app_name.upper() }}_NAUTOBOT_VER` variable.
