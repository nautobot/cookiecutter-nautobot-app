...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_{{ cookiecutter.nautobot_app_name.upper() }}_xxx
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.{{ cookiecutter.nautobot_app_name }}.compose_http_timeout,
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.{{ cookiecutter.nautobot_app_name }}.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations {{ cookiecutter.nautobot_app_name }}"
...
def dbshell(context, query="", input="", output=""):
    ...
    if "docker-compose.mysql.yml" in context.{{cookiecutter.nautobot_app_name}}.compose_files:
    ...
    elif "docker-compose.postgres.yml" in context.{{cookiecutter.nautobot_app_name}}.compose_files:
        command += [
            "--",
            service,
            "psql",
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml {{ cookiecutter.nautobot_app_name }}'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="{{cookiecutter.nautobot_app_name}}", failfast=False, buffer=True, pattern=""):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include '{{ cookiecutter.nautobot_app_name }}/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.{{ cookiecutter.nautobot_app_name }}.local):
