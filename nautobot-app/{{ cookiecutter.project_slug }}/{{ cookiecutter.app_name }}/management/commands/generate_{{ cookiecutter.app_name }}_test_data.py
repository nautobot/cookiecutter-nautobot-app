"""Generate test data for the {{ cookiecutter.verbose_name }} app."""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--flush",
            action="store_true",
            help="Flush any existing {{ cookiecutter.verbose_name }} test data from the database before generating new data.",
        )

    def _generate_static_data(self, db):
        """Generate static data required for test cases."""
        # TODO: populate the database with {{ cookiecutter.verbose_name }} test data.

    def handle(self, *args, **options):
        """Entry point to the management command."""
        if options["flush"]:
            self.stdout.write("Flushing existing {{ cookiecutter.verbose_name }} test data...")
