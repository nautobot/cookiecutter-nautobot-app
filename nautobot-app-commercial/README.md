...
- This cookie wires up the private `artifactory-pypi` Poetry source (`priority = "explicit"`, so only packages that declare it are resolved from it) and the credential plumbing to authenticate against it (Docker BuildKit secrets in `development/Dockerfile` and `development/docker-compose.base.yml`, `ARTIFACTORY_USERNAME`/`ARTIFACTORY_PASSWORD` resolved from the shell environment or `development/creds.env` in `tasks.py`, and step-scoped `POETRY_HTTP_BASIC_ARTIFACTORY_PYPI_*` credentials in CI). To depend on a package published only to Artifactory, uncomment the example in `pyproject.toml` and add `source = "artifactory-pypi"` to the dependency. Everything degrades gracefully when credentials are absent, so a cookie with no private dependencies builds without any setup.
